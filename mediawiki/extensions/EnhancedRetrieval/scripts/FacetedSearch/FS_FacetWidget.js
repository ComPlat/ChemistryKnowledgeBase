 ...
 */
console.log("ER: Loading scripts/FacetedSearch/FS_FacetWidget.js");
...
FacetedSearch.classes.FacetWidget = AjaxSolr.AbstractFacetWidget.extend({
	...
	// [array of Function]
	mPropertyFacetListener: [],
	
	addPropertyFacetListener: function(listener) {
		this.mPropertyFacetListener.push(listener);
	},
	...
	showFacetsForGroup : function (group) {
		...
		for (var i = start, l = end; i < l; i++) {
			...
			var show = true;
			for(var lis = 0; lis < this.mPropertyFacetListener.length; lis++) {
				show &= this.mPropertyFacetListener[lis](facetItem);
			}
			if (show) {
