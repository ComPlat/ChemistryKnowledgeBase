...
class ExperimentXlsImporter
{
        ...
        do {
            ...
            if ($rowNotEmpty) {
                ...
                foreach ($aggregatedValues as $value) {
                    ...
                    if (str_starts_with($value['templateParam'], "auto-generated-")) {
                        continue;
                    }
                    ...
                    $rowsContent .= "\n|{$value['templateParam']}={$templateValue}";
    ...
    private function createHeaderData($property): ?array
    {
        if ($property === 'BasePageName') return null;
        preg_match('/<([^>]+)>/', $property, $matches);
        if (!isset($matches) || count($matches) === 0) {
            return $this->makeHeaderData($property, $property);
        }
        $templateName = $matches[1];
        $property = trim(str_replace($matches[0], "", $property));
        return $this->makeHeaderData($templateName, $property);
    }

    private function makeHeaderData($templateName, $property): ?array
    {
