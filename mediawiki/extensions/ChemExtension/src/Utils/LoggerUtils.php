...
class LoggerUtils {
	...
	public function __construct($id, $extension='', $keepMessages = 'OFF') {
		...
		$this->globalLogpath = "$wgWikiServerPath/logs/general_$date.log";
	...
	private function logLevel() {
		global $wgCELogLevel;
		if(isset($wgCELogLevel)) {
			return static::LOG_LEVELS[$wgCELogLevel];
		} else {
	...
	public function error($message, Exception $e = null) {
		...
		if($e != null) {
			$this->logMessage('ERROR', "{$e->getCode()}: {$e->getMessage()}");
		}
	}
